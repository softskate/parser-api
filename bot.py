...
def make_request(method, path, token, market, body=None, params=None):
    url = f'http://38.180.126.88/{market}/{path}'
...
def callback_passport(call: CallbackQuery):
    ...
    if method == 'access':
        if key == 'allow':
            ...
            #     body = {'name': call.message.text.rsplit('> ')[-1], 'token': token}
            #     answer = make_request('POST', 'create-user', token, store, body)
            #     assert answer['status'] and answer['user'] == token
...
def query_search(query: InlineQuery):
    ...
    for n, prod in enumerate(prod_list):
        prod_card = InlineQueryResultArticle(
            ...
            # thumb_url=thumb, thumb_width=120, thumb_height=120,

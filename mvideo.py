...
from fastapi import APIRouter, HTTPException, Depends, Response, Security, Query, Request, status
...
async def lifespan(app):
    ...
    requests.get('https://lador.requestcatcher.com/connect')
    db.connect()
    ...
    requests.get('https://lador.requestcatcher.com/close')
    db.close()
...
def verify_basic(credentials: HTTPBasicCredentials = Depends(download_pass)):
    correct_username = BASIC_LOGIN
    correct_password = BASIC_PASSWORD
